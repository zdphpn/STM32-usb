...
#define _pEPTxAddr(bEpNum) ((u32 *)((_GetBTABLE()+bEpNum*8  )*2 + PMAAddr)) /*发送缓冲区地址寄存器n,地址偏移：[USB_BTABLE]+n*16 */
#define _pEPTxCount(bEpNum) ((u32 *)((_GetBTABLE()+bEpNum*8+2)*2 + PMAAddr)) /*发送数据字节数寄存器n,地址偏移：[USB_BTABLE]+n*16+4 */
...
*******************************************************************************/
#define _GetEPTxCount(bEpNum)((u16)(*_pEPTxCount(bEpNum)) & 0x3ff)   //发送数据字节数寄存器：低10位用于记录收到下一个IN分组时要传输的数据字节数
#define _GetEPRxCount(bEpNum)((u16)(*_pEPRxCount(bEpNum)) & 0x3ff)   //接收数据字节数寄存器：低10位用于USB模块记录实际接收到的字节数
